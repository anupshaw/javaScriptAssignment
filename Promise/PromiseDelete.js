...
function getPosts(){
   ...
   timeOut=setInterval(()=>{
      let output='';
   posts.forEach((post)=>{
      output+= `<li>${post.title} - ${post.body} - createdAt ${(new Date().getTime()-post.createdAt)/1000} sec ago<button class='delete' onclick=deletePost().then(deletea(e))>Delete</button></li>`
